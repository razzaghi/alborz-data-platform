...
python /alborz/backend/manage.py migrate
if [ -n "$ADMIN_USERNAME" ]; then
    python /alborz/backend/manage.py create_admin --noinput --username="$ADMIN_USERNAME" --email="$ADMIN_EMAIL" --password="$ADMIN_PASSWORD"
fi
