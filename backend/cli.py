...
env = Env()
ALBORZ_HOME = os.path.expanduser(os.environ.get("ALBORZ_HOME", "~/alborz-data-platform"))
Path(ALBORZ_HOME).mkdir(parents=True, exist_ok=True)
env.bool("DEBUG", False)
os.environ["STANDALONE"] = "True"
os.environ.setdefault("DJANGO_SETTINGS_MODULE", "config.settings.production")
os.environ.setdefault("DATABASE_URL", os.path.join(f"sqlite:///{ALBORZ_HOME}", "db.sqlite3"))
os.environ.setdefault("MEDIA_ROOT", os.path.join(ALBORZ_HOME, "media"))
base = os.path.abspath(os.path.dirname(__file__))
sys.path.append(base)
parser = argparse.ArgumentParser(description="Alborz, annotation tool for machine learning practitioners.")
